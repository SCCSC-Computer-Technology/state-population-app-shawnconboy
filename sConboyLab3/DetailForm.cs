            ...
            foreach (DataRow row in this.stateInformationDBDataSet.StateInformation.Rows)
            {
                if (row["Name"].ToString() == selectedState)
                {
                    ...
                    populationTextBox.Text =
                        Convert.ToInt32(row["Population"]).ToString("N0");

                    flag_DescriptionTextBox.Text =
                        row["Flag_Description"].ToString();

                    flowerTextBox.Text = row["Flower"].ToString();
                    birdTextBox.Text = row["Bird"].ToString();
                    colorsTextBox.Text = row["Colors"].ToString();
                    three_Largest_CitiesTextBox.Text =
                        row["Three_Largest_Cities"].ToString();

                    capitolTextBox.Text = row["Capitol"].ToString();

                    median_IncomeTextBox.Text =
                        Convert.ToDecimal(row["Median_Income"]).ToString("C0");

                    percent_Tech_JobsTextBox.Text =
                        Convert.ToDecimal(row["Percent_Tech_Jobs"]).ToString("0.0") + "%";
